...
class MedicalWebcamDetector:
    ...
    def __init__(self):
        ...
        # Set MLflow tracking URI (local directory or your server)
        ...
        self.mlflow_experiment_name = "MedicalFaceMaskWebcam"
        self.mlflow_experiment_id = self.get_or_create_mlflow_experiment(self.mlflow_experiment_name)

        # Initialize predictor
        self.init_predictor()

    def get_or_create_mlflow_experiment(self, experiment_name):
        """Get MLflow experiment ID by name, or create it if it doesn't exist."""
    ...
    def save_detection(self, frame, detection: dict):
        """Save high-confidence detections and log to MLflow experiment."""
        try:
            if detection['confidence'] >= 0.8:
                ...
                with mlflow.start_run(run_name=f"webcam_{base_filename}", experiment_id=self.mlflow_experiment_id):
    ...
    def draw_ui_panels(self, frame: np.ndarray):
        ...
        try:
            ...
            cv2.putText(frame, title_text, (20, 30), self.fonts['title'], 1.2, self.colors['accent'], 3)
            status_text = f"FPS: {self.current_fps} | Frame: {self.frame_count} | STATUS: ACTIVE"
            cv2.putText(frame, status_text, (20, 60), self.fonts['label'], 0.6, self.colors['text'], 2)
            scanner_y = int((time.time() * 100) % h)
            cv2.line(frame, (0, scanner_y), (w, scanner_y), self.colors['accent'], 2)
            panel_width = 280
            cv2.rectangle(frame, (w - panel_width, header_height), (w, h - 120), self.colors['panel'], -1)
            cv2.rectangle(frame, (w - panel_width, header_height), (w, h - 120), self.colors['accent'], 2)
            y_pos = header_height + 30
            cv2.putText(frame, "DETECTION STATUS", (w - panel_width + 10, y_pos), self.fonts['label'], 0.7, self.colors['accent'], 2)
            ...
            y_pos += 40
            cv2.putText(frame, f"Total Scans: {total}", (w - panel_width + 10, y_pos), self.fonts['label'], 0.5, self.colors['text'], 1)
            y_pos += 25
            cv2.putText(frame, f"Compliant: {compliant}", (w - panel_width + 10, y_pos), self.fonts['label'], 0.5, self.colors['success'], 1)
            y_pos += 25
            cv2.putText(frame, f"Non-Compliant: {non_compliant}", (w - panel_width + 10, y_pos), self.fonts['label'], 0.5, self.colors['error'], 1)
            y_pos += 25
            cv2.putText(frame, f"Improper: {improper}", (w - panel_width + 10, y_pos), self.fonts['label'], 0.5, self.colors['warning'], 1)
            y_pos += 35
            cv2.putText(frame, f"Compliance: {compliance_rate:.1f}%", (w - panel_width + 10, y_pos), self.fonts['label'], 0.6, self.colors['accent'], 2)
            panel_height = 120
            cv2.rectangle(frame, (0, h - panel_height), (w, h), self.colors['panel'], -1)
            cv2.rectangle(frame, (0, h - panel_height), (w, h), self.colors['success'], 2)
            y_start = h - 100
            cv2.putText(frame, "DETECTION STATISTICS", (20, y_start), self.fonts['stats'], 0.6, self.colors['accent'], 2)
            cv2.putText(frame, f"Total Detections: {total}", (20, y_start + 25), self.fonts['stats'], 0.5, self.colors['text'], 1)
            cv2.putText(frame, f"Compliance Rate: {compliance_rate:.1f}%", (20, y_start + 45), self.fonts['stats'], 0.5, self.colors['success'], 1)
            col2_x = 300
            cv2.putText(frame, "CLASSIFICATION BREAKDOWN", (col2_x, y_start), self.fonts['stats'], 0.6, self.colors['accent'], 2)
            cv2.putText(frame, f"Compliant: {compliant}", (col2_x, y_start + 25), self.fonts['stats'], 0.5, self.colors['success'], 1)
            cv2.putText(frame, f"Non-Compliant: {non_compliant}", (col2_x, y_start + 45), self.fonts['stats'], 0.5, self.colors['error'], 1)
            cv2.putText(frame, f"Improper: {improper}", (col2_x, y_start + 65), self.fonts['stats'], 0.5, self.colors['warning'], 1)
            col3_x = 600
            ...
            cv2.putText(frame, "SESSION INFORMATION", (col3_x, y_start), self.fonts['stats'], 0.6, self.colors['accent'], 2)
            cv2.putText(frame, f"Duration: {duration_str}", (col3_x, y_start + 25), self.fonts['stats'], 0.5, self.colors['text'], 1)
            cv2.putText(frame, f"Frame: {self.frame_count}", (col3_x, y_start + 45), self.fonts['stats'], 0.5, self.colors['text'], 1)
            controls_x = w - 300
            cv2.putText(frame, "CONTROLS", (controls_x, y_start), self.fonts['stats'], 0.6, self.colors['accent'], 2)
            cv2.putText(frame, "Q/ESC: Quit", (controls_x, y_start + 25), self.fonts['stats'], 0.4, self.colors['text'], 1)
            cv2.putText(frame, "S: Save Frame", (controls_x, y_start + 40), self.fonts['stats'], 0.4, self.colors['text'], 1)
            cv2.putText(frame, "R: Reset Stats", (controls_x, y_start + 55), self.fonts['stats'], 0.4, self.colors['text'], 1)
        except Exception as e:
    ...
    def draw_detection_box(self, frame, detection: Dict):
        ...
        try:
            if 'bbox' in detection:
                ...
                cv2.rectangle(frame, (label_bg_x1, label_bg_y1), (label_bg_x2, label_bg_y2), self.colors['text'], 1)
                cv2.putText(frame, label, (int(x1) + 5, int(y1) - 10), self.fonts['label'], 0.7, self.colors['text'], 2)
                conf_bar_width = int((x2 - x1) * confidence)
                cv2.rectangle(frame, (int(x1), int(y2) + 5), (int(x1) + conf_bar_width, int(y2) + 15), color, -1)
        except Exception as e:
        ...
        try:
            ...
            if hasattr(cv2, "destroyAllWindows"):
                try:
                    ...
                    print(f"⚠️ OpenCV GUI cleanup skipped: {e}")
            ...
            if self.analytics['total_detections'] > 0:
                print("Session ended with detections.")
